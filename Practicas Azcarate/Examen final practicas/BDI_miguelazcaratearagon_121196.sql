...
/*6*/
SELECT C.NOMBRE, C.CURVAS, C.CIUDAD, P.NOMBRE_ESP
FROM CIRCUITO C, PAIS P
WHERE (C.CODPAIS = P.CODPAIS)
ORDER BY C.NOMBRE;


/*7*/
SELECT MIN(C.CURVAS) AS MENOS_CURVAS
FROM CIRCUITO C;

/*8*/
SELECT PI.NOMBRE, MIN(PA.POSICION_LLEGADA)
FROM PILOTO PI, PARTICIPA PA
WHERE (PI.CODPILOTO = PA.CODPILOTO)
GROUP BY PI.NOMBRE
ORDER BY MIN(PA.POSICION_LLEGADA) ASC;

/*9*/
SELECT C.FECHACARRERA, C.NOMBRE_GP
FROM CARRERA C, PILOTO PI, PARTICIPA PA
WHERE (C.VUELTAS > 35) AND (C.FECHACARRERA = PA.FECHACARRERA) AND (PA.CODPILOTO = PI.CODPILOTO) AND (PI.NOMBRE LIKE 'Katherine Legge')
ORDER BY C.FECHACARRERA ASC;

/* 10 */
SELECT E.NOMBRE, PE.PERIODO, PA.NOMBRE_ENG, COUNT (PI.CODPILOTO)
